...
class PokemonBloc extends Bloc<PokemonEvent, PokemonState> {
  ...
  PokemonBloc({required this.pokemonDataSource}) : super(Initial()) {
    on<GetPokemons>((event, emit) async {
      ...
      final pokemonDetails = await pokemonDataSource.getPokemonDetails();
      try {
        ...
        emit(PokemonState.loaded(pokemons: pokemon, pokemonDetails: pokemonDetails));
      } catch (e) {
    ...
    on<LoadMore>(((event, emit) async {
      ...
      try {
        ...
        final List<Pokemon> pokemons = (state as Loading).oldPokemons.toList();

        pokemons.addAll(newPokemons);
        emit(PokemonState.loaded(pokemons: newPokemons, pokemonDetails: pokemonDetails));
      } catch (e) {
        return emit(PokemonState.error());
      }
    }));
